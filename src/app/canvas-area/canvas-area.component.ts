...
export class CanvasAreaComponent implements AfterViewInit {
        ...
        this.canvas.nativeElement.addEventListener(
            ...
            (e) => {
                if (e.touches.length === 2) {
                    ...
                    const zoomFactor =
                        currentDistance > this.startDistance ? zoomStep : 1 / zoomStep;
                    const newScale = Math.max(0.5, Math.min(2, this.scale * zoomFactor));
                    if (newScale === this.scale) return;
                    this.scale = newScale;
                    if (this.isMobile) {
                        if (this.shirtSideFront) {
                            this.imageWidth = Math.round(
                                Math.max(
                                    50,
                                    Math.min(this.canvasWidthFrontMob, this.imageWidth * this.scale)
                                )
                            );
                            ...
                            this.imageWidth = Math.round(
                                Math.max(
                                    50,
                                    Math.min(this.canvasWidthBackMob, this.imageWidth * this.scale)
                                )
                            );
                            this.imageHeight = Math.round(
                                Math.max(
                ...
                if (this.isMobile) {
                    if (this.shirtSideFront) {
                        ...
                        this.imageHeight = Math.round(
                            Math.max(
                                50,
                                Math.min(this.canvasHeightFrontMob, this.imageHeight * this.scale)
                            )
                        );
                    } else {
                        this.imageWidth = Math.round(
                        ...
                        this.imageHeight = Math.round(
                            Math.max(
                                50,
                                Math.min(this.canvasHeightBackMob, this.imageHeight * this.scale)
                            )
                        );
                    }
                }
    ...
    handleFileChange(event: any, from = '') {
        ...
        if (from == 'changeShirtSide') {
            ...
            let targetfile = this.shirtSideFront
                ? this.imageFrontGlobalTarget
                : this.imageBackGlobalTarget;
    ...
    updateImagePosition(deltaX: number, deltaY: number) {
        this.imageOffsetX = Math.max(
            0,
            Math.min(this.imageOffsetX + deltaX, this.canvas.nativeElement.width - this.imageWidth)
        );
